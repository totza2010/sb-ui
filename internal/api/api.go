...
func Mount(r chi.Router) {
	...
	// File-integrity audit (read-only against the teldrive Postgres, DSN in Options)
	r.Get("/api/teldrive/config", teldriveGetConfig)
	r.Put("/api/teldrive/config", teldrivePutConfig)
	r.Post("/api/teldrive/db/test", teldriveDBTest)
	r.Get("/api/teldrive/audit", teldriveAuditHandler)
